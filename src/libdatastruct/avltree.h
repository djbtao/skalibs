...
#define AVLTREE_ZERO { .x = GENSETDYN_ZERO, .root = (unsigned int)-1, .cmp = 0, .external = 0 }
...
#define AVLTREE_INIT(b, num, den, f, p) { .x = GENSETDYN_INIT(avlnode, (b), num, den), .root = (unsigned int)-1, .cmp = (f), .external = (p) }
